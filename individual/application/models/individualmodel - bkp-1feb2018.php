...
function password_change()
   {
    ...
    if(isset($_POST['passchange'])){
      ...
      if(!$chk){
        ...
        if($this->db->update('ind_registration',$data,array('username'=>$this->session->userdata('login')))) {
    ...
    function forgot_password()
    {
      if(isset($_POST['fpassword']))
      {
        ...
        if($this->db->get_where('ind_registration',array('email1'=>$chkemail))->row())
        {
            ...
            $message = "Password Reset Link: ".base_url('forgot_password').'?'.$hash;
            $this->fun->sendMail($chkmail,$message);
        } else {
  ...
  function update_profile(){
      ...
      $scanchk = $this->db->get_where('ind_registration',array('id'=>$this->session->userdata('login')))->row();
    ...
    if($this->db->update('ind_registration',$data,array('id'=>$this->session->userdata('login'))))
    {
       $pfd=$this->db->list_fields('ind_registration');
       $pchk=$this->db->get_where('ind_registration',array('id'=>$this->session->userdata('login')))->row();
       foreach($pfd as $fd){
       if($pchk->$fd=='')
        { 
...
function all_family_members(){
	...
	$member_id=$this->session->userdata('login');
	return $this->db->get_where('ind_family',array('member_id'=>$member_id))->result();
...
if($this->db->update('ind_family',$data,array('id'=>$this->uri->segment(2))))
{
      $pfd=$this->db->list_fields('ind_family');
       $pchk=$this->db->get_where('ind_family',array('member_id'=>$this->session->userdata('login')))->row();
       foreach($pfd as $fd){
       if($pchk->$fd=='')
        { 
...
function all_requests(){
	...
	$member_id=$this->fun->getnewfieldvalue('ind_registration','id','username',$this->session->userdata('login'));
	return $this->db->get_where('ind_service_request',array('member_id'=>$member_id))->result();
      ...
      if($this->db->insert('ind_share_app',$data)) 
          {
				...
				$message = "Hi,to get finest services & maintain all your insurance policies at one single place I strongly recommend you to load Mallapuram Personal Insurance Portfolio APP. http://elefontdesign.in/client/mallapuram_corporate/individual/";
       ...
       $this->fun->sendSMS($mobile,$message);
			   ...
			   return $msg = '<div class="alert alert-success col-md-12">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                    <strong>Share App Added Successfully</strong>
                  </div>';
